...
idSnapShot.prototype._Release = function() {
    //assert( mem.IsMapHeap() );
    // if ( data != NULL ) {
    if ( this.data !== null ) {
        // assert( size > 0 );
...
*/
// idSnapShot::idSnapShot() :
idSnapShot.prototype.constructor = function() {
    // time( 0 ),
    this.time = 0;
    // recvTime( 0 )
    this.recvTime = 0;
...
ReadDelta: function( file, visIndex ) {

    // file->ReadBig( time );
    this.time = file.ReadNumber();
    ...
    while ( true ) {
        // objectNum += delta;
        var objectNum = file.ReadString();
        // if ( objectNum >= 0xFFFF ) {
        if ( objectNum === 'FFFFFFFF-FFFF-FFFF-FFFF-FFFFFFFFFFFF' ) {
            // full delta
            return true;
        }
        // objectState_t & state = FindOrCreateObjectByID( objectNum );
        var state = this.FindOrCreateObjectByID( objectNum );
        // objectSize_t newsize = 0;
        // file->ReadBig( newsize );
        var newsize = file.ReadNumber();
        ...
        */
        ...
        if ( newsize === 0 ) {
            ...
            // objectBuffer_t newbuffer( newsize );
            var newbuffer = new objectBuffer_t( newsize );
            // objectSize_t compareSize = Min( newsize, state.buffer.Size() );
            var compareSize = Math.min( newsize, state.buffer.Size() );
            ...
            for ( var i = 0; i < compareSize; i++ ) {
                ...
                delta = file.ReadNumber() & 0xFF;
                // newbuffer[i] = state.buffer[i] + delta;
                newbuffer.data[i] = state.buffer.data[i] + delta;
            }
        ...
        if( typeof SNAPSHOT_CHECKSUMS !== 'undefined' && SNAPSHOT_CHECKSUMS ) {
            if ( state.buffer.Size() > 0 ) {
                ...
                var checksum = file.ReadString();
                // assert( checksum == MD5_BlockChecksum( state.buffer.Ptr(), state.buffer.Size() ) );
                console.assert( checksum === SparkMD5.ArrayBuffer.hash( state.buffer.Ptr() ) );
            }
        // #endif
        }
    }
...
WriteObject: function( file, visIndex, newState, oldState ) {
    // assert( newState != NULL || oldState != NULL );
    console.assert( newState !== null || oldState !== null );
    ...
    */
    ...
    if ( newState !== null && oldState !== null ) {
        ...
        console.assert( newState.objectNum === oldState.objectNum );
    ...
    // } else if ( oldState == NULL ) {
        ...
        console.assert( newState !== null );
        // delta against an empty snap
        // file->WriteBig( newState->buffer.Size() );
        file.WriteNumber( newState.buffer.Size() );
        // file->Write( newState->buffer.Ptr(), newState->buffer.Size() );
        file.Write( newState.buffer.Ptr(), 0, newState.buffer.Size() );
    } else {
        ...
        */
        if ( !visChange || visSendState ) {
            ...
            var compareSize = Math.min( newState.buffer.Size(), oldState.buffer.Size() );
            ...
            for ( var b = 0; b < compareSize; b++ ) {
                // file->WriteBig<byte>( ( 0xFF + 1 + newState->buffer[b] - oldState->buffer[b] ) & 0xFF );
                file.WriteNumber( ( 0xFF + 1 + newState.buffer.data[b] - oldState.buffer.data[b] ) & 0xFF );
            }
    ...
    if( typeof SNAPSHOT_CHECKSUMS !== 'undefined' && SNAPSHOT_CHECKSUMS ) {
        ...
        if ( ( !visChange || visSendState ) && newState !== null ) {
            ...
            console.assert( newState.buffer.Size() > 0 );
            // unsigned int checksum = MD5_BlockChecksum( newState->buffer.Ptr(), newState->buffer.Size() );
            var checksum = SparkMD5.ArrayBuffer.hash( newState.buffer.Ptr() );
            // file->WriteBig( checksum );
            file.WriteString( checksum ); // TODO: write as integer
        }
    }
    // #endif
};
...
WriteDelta: function( old, visIndex, file, maxLength, optimalLength ) {
    // file->WriteBig( time );
    file.WriteNumber( this.time );

    // int objectHeaderSize = sizeof( uint16 ) + sizeof( objectSize_t );
// #ifdef SNAPSHOT_CHECKSUMS
    // TODO: implement objectHeaderSize checks
...
// #endif
    ...
    // for ( int i = 0; i < objectStates.Num(); i++ ) {
    for ( var i = 0; i < this.objectStates.length; i++ ) {
        // objectState_t & newState = *objectStates[i];
        var newState = this.objectStates[i];
        ...
        */
        ...
        if ( newState.buffer.Size() > maxLength ) {
            ...
            console.log( "Snap obj [" + newState.objectNum + "] state.size > max packet length. Skipping... " );
            continue;
        // } else if ( !verify( newState.buffer.Size() > 0 ) ) {
        } else if ( newState.buffer.Size() <= 0 ) {
            // you CANNOT have a valid ss obj state w/ size = 0: this will be interpreted as a delete in ::ReadDelta and this will completely throw
            // off delta compression, eventually resulting in a checksum error that is a pain to track down.
            // idLib::Warning( "Snap obj [%d] state.size <= 0... skipping ", newState.objectNum );
            console.log( "Snap obj [" + newState.objectNum + "] state.size <= 0... skipping " );
            continue;
        }
        ...
        */
        ...
        if ( j >= old.objectStates.length ) {
            // delta against an empty snap
            // WriteObject( file, visIndex, &newState, NULL, lastobjectNum );
            this.WriteObject( file, visIndex, newState, null );
            continue;
        }
        ...
        for ( ; newState.objectNum > old.objectStates[j].objectNum; j++ ) {
            ...
            */
            ...
            oldState = old.objectStates[j];
            // WriteObject( file, visIndex, NULL, &oldState, lastobjectNum );
            this.WriteObject( file, visIndex, null, oldState );
        }
        ...
        if ( newState.objectNum === oldState.objectNum ) {
            ...
            */
            ...
            this.WriteObject( file, visIndex, newState, oldState );
            j++;
        } else {
            /*
            if ( file->Length() + objectHeaderSize + newState.buffer.Size() >= maxLength ) {
    ...
    for ( ; j < old.objectStates.length; j++ ) {
        /*
        if ( file->Length() + objectHeaderSize >= maxLength ) {
            return false;
        }
    ...
    */
    // file->WriteBig( objectDelta );
    file.WriteString( 'FFFFFFFF-FFFF-FFFF-FFFF-FFFFFFFFFFFF' );
...
idSnapShot.prototype.S_AddObject = function( objectNum, visMask, data, _size, tag ) {
    ...
    if( data instanceof idBitMsg ) {
        var msg = data;
        tag = _size;
        return this.S_AddObject( objectNum, visMask, msg.GetReadData(), msg.GetSize(), tag );
    }
    ...
    if ( state.buffer.Size() === size && state.buffer.NumRefs() === 1 ) {
        ...
        memcpy( state.buffer.Ptr(), 0, data, 0, size );
    } else {
        // objectBuffer_t buffer( size );
        var buffer = new objectBuffer_t( size );
        // memcpy( buffer.Ptr(), data, size );
        memcpy( buffer.Ptr(), 0, data, 0, size );
        state.buffer = buffer;
    }
    // return &state;
    return state;
...
idSnapShot.prototype.GetObjectMsgByIndex = function( i, msg, ignoreIfStale ) {
    // if ( i < 0 || i >= objectStates.Num() ) {
    if ( i < 0 || i >= this.objectStates.length ) {
        return -1;
    }
    // objectState_t & state = *objectStates[i];
    var state = this.objectStates[i];
    if ( state.stale && ignoreIfStale ) {
        return -1;
    }
    // msg.InitRead( state.buffer.Ptr(), state.buffer.Size() );
    msg.InitRead( state.buffer.data );
...
idSnapShot.prototype.BinarySearch = function( objectNum ) {
    // int lo = 0;
    var lo = 0;
    // int hi = objectStates.Num();
    var hi = this.objectStates.length;
    // while ( hi != lo ) {
    while ( hi !== lo ) {
        // int mid = ( hi + lo ) >> 1;
        var mid = ( hi + lo ) >> 1;
        ...
        if ( this.objectStates[mid].objectNum === objectNum ) {
...
idSnapShot.prototype.FindOrCreateObjectByID = function( objectNum ) {
    //assert( mem.IsMapHeap() );

    // int i = BinarySearch( objectNum );
    var i = this.BinarySearch( objectNum );

    // if ( i >= 0 && i < objectStates.Num() && objectStates[i]->objectNum == objectNum ) {
    if ( i >= 0 && i < this.objectStates.length && this.objectStates[i].objectNum === objectNum ) {
        // return *objectStates[i];
        return this.objectStates[i];
    }

    // objectState_t * newstate = allocatedObjs.Alloc();
    var newstate = new objectState_t();
    // newstate->objectNum = objectNum;
    newstate.objectNum = objectNum;

    // objectStates.Insert( newstate, i );
    this.objectStates.splice( i, 0, newstate );
