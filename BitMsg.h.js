...
*/
    ...
    this.writeData = null;             // pointer to data for writing
    // const byte *    readData;       // pointer to data for reading
    this.readData = null;              // pointer to data for reading
    // int             maxSize;        // maximum size of message in bytes
    this.maxSize = 0;                  // maximum size of message in bytes
    // int             curSize;        // current size of message in bytes
    this.curSize = 0;                  // current size of message in bytes
    // mutable int     writeBit;       // number of bits written to the last written byte
    this.writeBit = 0;                 // number of bits written to the last written byte
    // mutable int     readCount;      // number of bytes read so far
    this.readCount = 0;                // number of bytes read so far
    // mutable int     readBit;        // number of bits read from the last read byte
    this.readBit = 0;                  // number of bits read from the last read byte
    // bool            allowOverflow;  // if false, generate error when the message is overflowed
    this.allowOverflow = false;        // if false, generate error when the message is overflowed
    // bool            overflowed;     // set true if buffer size failed (with allowOverflow set)
    this.overflowed = false;           // set true if buffer size failed (with allowOverflow set)
...
idBitMsg.prototype.GetReadData = function() {
    // return readData;
...
idBitMsg.prototype.GetMaxSize = function() {
    // return maxSize;
    return this.maxSize;
};
...
idBitMsg.prototype.GetSize = function() {
    // return curSize + ( writeBit != 0 );
    return this.curSize + ( this.writeBit !== 0 );
};
...
function GetNumBitsWritten() {
    // return ( curSize << 3 ) + writeBit;
    return ( this.curSize << 3 ) + this.writeBit;
}
...
idBitMsg.prototype.GetRemainingWriteBits = function() {
...
idBitMsg.prototype.GetNumBitsRead = function() {
...
idBitMsg.prototype.GetRemainingData = function() {
    // assert( writeBit == 0 );
    console.assert( this.writeBit === 0 );
...
idBitMsg.prototype.GetRemainingReadBits = function() {
    // assert( writeBit == 0 );
    console.assert( this.writeBit === 0 );
    // return ( curSize << 3 ) - GetNumBitsRead();
    return ( this.curSize << 3 ) - this.GetNumBitsRead();
};
...
idBitMsg.prototype.BeginWriting = function() {
    // curSize = 0;
    this.curSize = 0;
    // overflowed = false;
    this.overflowed = false;
...
idBitMsg.prototype.WriteByteAlign = function() {
    ...
    console.assert( this.writeBit === 0 || ( ( this.writeData[this.curSize - 1] >> this.writeBit ) === 0 ) ); // had to early out writeBit == 0 because when writeBit == 0 writeData[curSize - 1] may be the previous byte written and trigger false positives
    // writeBit = 0;
    this.writeBit = 0;
    // tempValue = 0;
    this.tempValue = 0;
...
idBitMsg.prototype.WriteByte = function( c ) {
    // WriteBits( c, 8 );
    this.WriteBits( c, 8 );
};
...
idBitMsg.prototype.ReadByteAlign = function() {
    // readBit = 0;
    this.readBit = 0;
};
...
idBitMsg.prototype.ReadByte = function() {
